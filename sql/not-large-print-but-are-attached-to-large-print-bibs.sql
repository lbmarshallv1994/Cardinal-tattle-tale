...
join lateral (
select * from lp_bibs 
where copy_id not in (select copy_id from tattler_ignore_list where system_id = system_id and report_name = ?) and (select id from actor.org_unit_ancestor_at_depth(circ_lib, 1)) = system_id
limit ?
) p on true
order by system_id;
